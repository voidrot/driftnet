...
try:
    ...
    logger.exception('Failed to import SDE models, ensure they are generated first')
...
class Command(BaseCommand):
    ...
    def get_remote_checksums(self):
        if not self.remote_checksums:
            res = httpx.get(app_settings.SDE_CHECKSUM_URL)
    ...
    def download_sde(self, force_download: bool) -> None:
        ...
        if force_download or needs_download:
            ...
            chunk_size = 1 * 1024**2
            with (
                httpx.stream('GET', app_settings.SDE_ARCHIVE_URL) as r,
    ...
    def build_file_path(self, sub_path: str, filename: str) -> tuple[Path, Path]:
        ...
        return (
            Path(app_settings.SDE_WORKSPACE / sub_path / f'{filename_base}.json'),
            Path(app_settings.SDE_WORKSPACE / sub_path / f'{filename_base}.yaml'),
        )
