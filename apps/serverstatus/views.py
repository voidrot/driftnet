...
def current_status(request: HttpRequest) -> HttpResponse:
    ctx = {
        ...
        'server_status_color': 'server-status-text-red',
        'current_players': 0,
        'player_count_color': 'server-status-text-red',
    }
    ...
    if server_status is None:
        ...
        if server_status.player_count < 15000:
            ctx['player_count_color'] = 'server-status-text-red'
        elif server_status.player_count < 20000:
            ctx['player_count_color'] = 'server-status-text-orange'
        else:
            ctx['player_count_color'] = 'server-status-text-green'
        ctx['current_players'] = server_status.player_count
        if server_status.vip_mode:
            ctx['server_status_text'] = 'VIP Mode'
            ctx['server_status_color'] = 'server-status-text-orange'
            ctx['current_players'] = server_status.player_count
        else:
            ctx['server_status_text'] = 'Online'
            ctx['server_status_color'] = 'server-status-text-green'
            ctx['current_players'] = server_status.player_count
